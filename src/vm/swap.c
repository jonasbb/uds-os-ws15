...
void init_swap() {
    ...
    swap_block_size = block_size(swap_block) /  sectors_per_page;
...
swap_get_next_free() {
    ...
    int swapmap_size = bitmap_size(swap_map);
    if (bitmap_all(swap_map, 0, swapmap_size)) {
        PANIC("SWAP full!");
    }
...
read_page_from_block(struct swaptable_entry * st_e, void *addr) {
    ...
    int i;  
    for (i= 0; i < sectors_per_page; i++){
            block_read(swap_block, st_e->swap_sector * sectors_per_page + i,
